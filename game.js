...
canvas.width = window.innerWidth * 0.80; // Set canvas width to 80% of window width
canvas.height = window.innerHeight * 0.80; // Set canvas height to 80% of window height

// Game Border
const worldWidth = 1700;
...
const playerCircle = {
    ...
    speed: 5,
    angle: 0,
    lives: 1, // lives that the player has
    lastShotTime: 0, // Time of the last shot
    shootCooldown: 100, // Cooldown in milliseconds
};
...
const enemyCircle = {
    radius: 20,
    lives: 1, // lives
    shootCooldown: 500, // 0.5 seconds cooldown
...
const maxDistortion = 5; // Maximum blur radius
...
function update() {
    ...
    if (distanceToEnemy < playerCircle.radius + enemyCircle.radius && !isPlayerInvincible) {
        ...
        // Activate flashing and invincibility for 0.5 seconds
        playerFlashTimer = 500;
        isPlayerInvincible = true;

        // Reset invincibility after 0.5 seconds
        setTimeout(() => {
        ...
        if (distanceSq < minDistance ** 2) {
            ...
            if (enemyCircle.lives <= 0) {
                ...
                enemyCircle.lives = 1; // Reset enemy lives
...
function draw() {
    ...
    for (let i = 0; i < playerCircle.lives; i++) {
        ...
        // Draw yellow outline for each life
        ctx.beginPath();
        ctx.arc(lifeX, livesY, lifeRadius + 2, 0, Math.PI * 2);
        ctx.stroke();

        // Draw green filled circle for each life
        ctx.beginPath();
    ...
    // Draw game world boundaries with cyan outline
    ctx.strokeStyle = 'cyan'; // Changed color
    ctx.lineWidth = 2;
    ctx.strokeRect(0, 0, worldWidth, worldHeight);

    // Draw bullets with a new color (purple)
    ...
    // Draw enemy circle with a new color (pink)
    ctx.beginPath();
    ctx.arc(enemyCircle.x, enemyCircle.y, enemyCircle.radius, 0, Math.PI * 2);
    ctx.fillStyle = 'pink'; // Changed color
    ctx.fill();

    // Draw enemy bullets with a new color (orange)
    enemyBullets.forEach(bullet => {
        ctx.save();
        ctx.translate(bullet.x, bullet.y);
        ctx.beginPath();
        ctx.arc(0, 0, 5, 0, Math.PI * 2);
        ctx.fillStyle = 'orange'; // Changed color
        ctx.fill();
        ctx.restore();
    });
    ...
    if (!isPlayerInvincible || (isPlayerInvincible && playerFlashTimer % 200 < 100)) {
        ...
        // Draw player circle with a new color (blue)
        ctx.save();
        ctx.translate(playerCircle.x, playerCircle.y);
        ctx.rotate(playerCircle.angle);
        ctx.beginPath();
        ctx.arc(0, 0, playerCircle.radius, 0, Math.PI * 2);
        ctx.fillStyle = 'blue'; // Changed color
        ctx.fill();
        ctx.restore();

        // Draw triangle with a new color (lime)
        ...
        ctx.lineTo(-triangle.size / 1, triangle.size / 2);
        ctx.closePath();
        ctx.fillStyle = 'lime'; // Changed color
        ctx.fill();
        ctx.restore();
    }
    ...
    // If player has no lives left, display game over with a new color (red)
    if (playerCircle.lives <= 0) {
        ctx.save();
        ctx.fillStyle = 'red'; // Changed color
        ctx.font = '48px Arial';
    ...
    // Draw "Paused" text with a new color (yellow) if the game is paused
    if (isPaused) {
        ctx.save();
        ctx.fillStyle = 'yellow'; // Changed color
        ctx.font = '48px Arial';
        ctx.textAlign = 'center';
